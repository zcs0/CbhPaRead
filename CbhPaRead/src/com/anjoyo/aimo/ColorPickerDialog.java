...
public class ColorPickerDialog extends Dialog {
	...
	private final String TAG = "91PandaReader";
	...
	private class ColorPickerView extends View {
		...
		public ColorPickerView(Context context, int height, int width) {
			super(context);
			this.mHeight = height - 35;
			this.mWidth = width;
			setMinimumHeight(height - 35);
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE:
				if (downInCircle && inCircle) {
					...
					if (debug)
						Log.v(TAG, "鑹茬幆鍐? 鍧愭爣: " + x + "," + y);
				} else if (downInRect && inRect) {
					mCenterPaint.setColor(interpRectColor(mRectColors, x));
				}
				if (debug)
					Log.v(TAG, "[MOVE] 楂樹寒: " + highlightCenter + "寰寒: "
							+ highlightCenterLittle + " 涓績: " + inCenter);
